...
class TreeSegmAffinity:
    ...
    def _load_config(self, config_dir: Optional[Union[pth.Path, str]] = None) -> dict:
        ...
        self._model_config["scaling_config"] = config_dict['scaling_config']
    ...
    def _model_predict(self, voxel: torch.Tensor) -> np.ndarray:
        ...
        voxel_probs = torch.sigmoid(voxel_probs)

        return voxel_probs.cpu().numpy()

    def _instance_segmentation(self, points, chunk_id=0):
        superpoints = []
        sp_indices = []
        
        if self.use_mp:
            sp_gen = build_superpoints_mp(points, n_jobs=self.n_jobs)
        else:
        ...
        for i, idx in enumerate(sp_gen):
            ...
            superpoints.append(SuperPoint(
                ...
                chunk_id=0,
                height_extent=points[idx, 2].max() - points[idx, 2].min()
            ))
            sp_indices.append(idx)
        
        centroids = np.array([sp.centroid for sp in superpoints])
        if self.use_mp and len(superpoints) > 1000:
            edges = build_edges_mp(centroids, radius=[0.3], n_jobs=self.n_jobs)
        else:
        ...
        for i, j in edge_iterator:
            ...
            score = self._model_predict(f.reshape(-1, 8))
...
def main():
    import laspy
    cloud = np.load("data/split/train/A1W_trees_000021.npy")
    points = cloud[:, :3]  # Use only XYZ coordinates
    labels = cloud[:, -1]
    

    SegmTrees = TreeSegmAffinity(model_name="AffinityMLPV1_2",
    ...
    labels = SegmTrees._instance_segmentation(points=points, chunk_id=0)


    print("Predicted labels:", np.unique(labels), labels.shape)
