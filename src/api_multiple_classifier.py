...
class LLM_Classifier:
    ...
    def api_call(self, images_base64: list[str]) -> int:
        ...
        system_prompt = (
            "<role>\n"
            "You are a dendrologist identifying tree species from 5 orthographic "
            "depth maps of a LiDAR point cloud (order: TOP, FRONT, BACK, LEFT, RIGHT). "
            "Brighter pixels are closer; each view is independently min-max normalised, "
            "so only shape is comparable across views.\n"
            "</role>\n\n"
            ...
            "Do not infer colour, bark, or leaf shape. Never invent a species.\n"
            "</reasoning_guidelines>\n\n"
            "<validation_rules>\n"
            "Return 19 if ANY holds: (1) multiple distinct trunks, "
            "(2) large parts of crown/trunk missing or cut off, "
            "(3) severe structural artefacts (low resolution alone does not count).\n"
            "</validation_rules>\n\n"
